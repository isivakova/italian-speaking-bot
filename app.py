...
downloads audio, converts to text via Claude vision,
...
def transcribe_audio(file_url: str) -> str:
    """Download audio and transcribe using a free workaround via Claude text prompt."""
    # Download the audio file
    audio_resp = requests.get(file_url, timeout=60)
    audio_bytes = audio_resp.content
    audio_b64 = base64.standard_b64encode(audio_bytes).decode("utf-8")

    # Use Claude with base64 audio as a document
    headers = {
    ...
    body = {
        ...
        "messages": [{
            ...
            "content": [
                ...
                {
                    "type": "text",
                    "text": """Sei un insegnante di italiano esperto per studenti di livello B2.
...
def webhook():
    ...
    if "voice" in message:
        ...
        try:
            ...
            full_response = transcribe_audio(file_url)

            # Split transcription from feedback
            if "━━━━━━━━━━━━━━━━━━━━" in full_response:
                parts = full_response.split("━━━━━━━━━━━━━━━━━━━━", 1)
                transcription = parts[0].replace("📝 TRASCRIZIONE:", "").strip()
                feedback = "🎓 FEEDBACK\n━━━━━━━━━━━━━━━━━━━━\n" + parts[1].strip()
            else:
                transcription = ""
